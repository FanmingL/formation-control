...
// 	sensor_setup.Offset.Gyro_Temperature = 0;
  ...
  pid_setup.groups.ctrl2.yaw.kd   = 0.1;
	...
	pid_setup.groups.hc_sp.kp = 1.8f;
	pid_setup.groups.hc_sp.ki = 1.0f;
	pid_setup.groups.hc_sp.kd = 1.0f;
	
	pid_setup.groups.hc_height.kp = 5.0f;
	pid_setup.groups.hc_height.ki = 0.03f;
	pid_setup.groups.hc_height.kd = 0.5f;	
...
//	int32_t result = Para_ReadSettingFromFile();		//commentted by Mr.Lin
//  if(result < 0)
//  {
//	 Para_ResetToFactorySetup();
//	 flash_init_error = 1;
//	}
	BSP_FLASH_Read(ADDR_FLASH_SECTOR_6,&ParaSavedFlag, 1);
	if (ParaSavedFlag==1){
		BSP_FLASH_Read(ADDR_FLASH_SECTOR_7,sensor_setup.raw_data, 64);

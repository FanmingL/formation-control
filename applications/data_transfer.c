...
s16 april_vx=0.0f,april_vy=0.0f;
s16 april_x=-100,april_y=-100;
void ANO_DT_Data_Exchange(void)
{
	...
	if((cnt % location_cnt) == (location_cnt-4))
	{
		f.send_location = 1;		
	}
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{
		f.send_senser = 0;
		ANO_DT_Send_Senser(mpu6050.Acc.x,mpu6050.Acc.y,mpu6050.Acc.z,mpu6050.Gyro.x,mpu6050.Gyro.y,mpu6050.Gyro.z,
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{
		f.send_senser2 = 0;
		ANO_DT_Send_Senser2(0,ultra.height);//原始数据
	}	
/////////////////////////////////////////////////////////////////////////////////////
	else if(f.send_rcdata)
	{
		f.send_rcdata = 0;
		ANO_DT_Send_RCData(CH[2]+1500,CH[3]+1500,CH[0]+1500,CH[1]+1500,global_mode_NS,Rc_Pwm_In[3],CH[6]+1500,CH[7]+1500,0 +1500,0 +1500);
	}	
/////////////////////////////////////////////////////////////////////////////////////	
	else if(f.send_motopwm)
	{
		f.send_motopwm = 0;
		ANO_DT_Send_MotoPWM(motor[0],motor[1],motor[2],motor[3],0,0,0,0);//motor[0-3](u16)april_x,(u16)april_y,(u16)april_vx,(u16)april_vy
	}	
/////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	else if(f.send_location == 1)
	{
		
		f.send_location = 0;
			f.send_location = 0;
		if (location_refresh){
		ANO_DT_Send_Location(fly_ready,NS,0 *10000000,0  *10000000,0,april_x,april_y,april_vx,april_vy);
		location_refresh=0;
		}
		else {
			ANO_DT_Send_Usart3(fly_ready,NS);
		}
...
/////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
	//Usb_Hid_Send();					commentted by Mr.Lin
/////////////////////////////////////////////////////////////////////////////////////
}
...
void ANO_DT_Data_Receive_Prepare2(u8 data)
{
	static u8 RxBuffer[50];
	static u8 _data_len = 0,_data_cnt = 0;
	static u8 state = 0;
...
void ANO_DT_Data_Receive_Prepare3(u8 data)
{
	static u8 RxBuffer[50];
	static u8 _data_len = 0,_data_cnt = 0;
	static u8 state = 0;
...
void ANO_DT_Send_Usart3(u8 lock,u8 NS)
{
	u8 _cnt=0;
	...
	data_to_send[_cnt++]=lock;								//上锁解锁
	data_to_send[_cnt++]=NS;							//模式
	u8 sum = 0;
	for(u8 i=0;i<_cnt;i++)
		sum += data_to_send[i];
	data_to_send[_cnt++]=sum;
	
		Usart3_Send(data_to_send,_cnt);
...
//		}
		...
		if(*(data_buf+2)==0X24){
			location_refresh=1;
			april_vx=(vs16)((*(data_buf+4)<<8)|*(data_buf+5));
			april_vy=(vs16) ((*(data_buf+6)<<8)|*(data_buf+7));
			april_x=(vs16)((*(data_buf+8)<<8)|*(data_buf+9));
			april_y=(vs16)((*(data_buf+10)<<8)|*(data_buf+11));
			//LED_Duty();
		}
		if(*(data_buf+2)==0X25){
		//	LED_Duty();
			height_guidance =(u16)((*(data_buf+4)<<8)|*(data_buf+5));
			//Ultra_Get(1);
			//LED_Duty();
		}
...
void ANO_DT_Send_Location(u8 state,u8 sat_num,s32 lon,s32 lat,float back_home_angle,s16 x,s16 y,s16 vx,s16 vy)
{
	u8 _cnt=0;
	vs16 _temp;
	vs32 _temp2;
	...
	data_to_send[_cnt++]=state;								//上锁解锁
	data_to_send[_cnt++]=sat_num;							//模式

...
//=========================== 期望角度 ========================================
	except_A.x  = MAX_CTRL_ANGLE  *( my_deathzoom( ( CH_filter[ROL]) ,5,1 )/500.0f );   //30
	except_A.y  = MAX_CTRL_ANGLE  *( my_deathzoom( (-CH_filter[PIT]) ,5,1 )/500.0f );  //30
	if( Thr_Low == 0 )
	{
		except_A.z += (s16)( MAX_CTRL_YAW_SPEED *( my_deathzoom_2( (CH_filter[YAW]) ,0,5 )/500.0f ) ) *T ;  //50
	}
	else
	{
